...
\midrule
Ingestão & Kinesis Data Streams (particionado por estação) \\
Arquivo bruto & Firehose $\rightarrow$ S3, GZIP, escalonado ao Glacier \\
Detecção & Lambda, \texttt{ReportBatchItemFailures} \\
Memória de \emph{replay} & DynamoDB, escrita condicional $+$ TTL \\
Plano de identidade & CloudTrail, eventos de gestão $+$ dados do S3 \\
Detecção nativa & GuardDuty, repontuado por um Lambda \\
Repositório de achados & Security Hub (ASFF~\cite{asff}) \\
Alerta & Métricas embutidas do CloudWatch, alarmes, SNS \\
\bottomrule
\end{tabular}
\end{table}
...
\input{data/table_effectiveness_pt}
